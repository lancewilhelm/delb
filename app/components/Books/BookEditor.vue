...
const currentBookId = ref((props.bookId ?? '').toString());

watch(
  () => props.bookId,
  (next) => {
    if (isCreateMode.value) return;
    currentBookId.value = (next ?? '').toString();
  },
);

const activeBookId = computed(() => currentBookId.value);
...
const book = ref<Book | null>(isCreateMode.value ? makeEmptyBook() : null);
...
function normalizeCollectionIds(input: string[] | undefined): string[] {
  return Array.from(
    new Set(
      (input ?? [])
...
const createCollectionIds = ref<string[]>(
  normalizeCollectionIds(props.initialCollectionIds),
);

watch(
  () => props.initialCollectionIds,
  (next) => {
    createCollectionIds.value = normalizeCollectionIds(next);
  },
);
...
const titleMissing = computed(() => !form.title.trim());
const authorsMissing = computed(
  () => authorChips.value.length === 0 && !authorInput.value.trim(),
);
const saveDisabled = computed(
  () => saving.value || titleMissing.value || authorsMissing.value,
);
const saveLabel = computed(() => (isCreateMode.value ? 'Create' : 'Save'));
const backTooltip = computed(() =>
  isCreateMode.value ? 'Back to library' : 'Back to book',
);
const displayTitle = computed(() => {
  const t = form.title.trim();
  if (t) return t;
  return book.value?.title || 'Untitled';
});
...
async function loadBook() {
  ...
  try {
    const res = await fetch(
      `/api/books/${encodeURIComponent(activeBookId.value)}`,
      {
        method: 'GET',
        headers: {
          Accept: 'application/json',
...
function commitPendingChips() {
  ...
  const committedTags = chipCommitOnEnter(tagChips.value, tagInput.value);
  if (committedTags.committed) {
    tagChips.value = committedTags.chips;
    tagInput.value = committedTags.input;
    tagSuggestOpen.value = false;
  }
}
...
function validateRequiredFields(payload: ReturnType<typeof buildPayload>) {
  if (!payload.title) {
...
async function createBook() {
  ...
  try {
    ...
    const res = await fetch('/api/books', {
      ...
      body: JSON.stringify({
        ...payload,
        collectionIds: createCollectionIds.value.length
          ? createCollectionIds.value
          : undefined,
      }),
    });
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => '');
      setError(text || `Failed to create (${res.status}).`);
      return;
    }
    ...
    const createdId = json?.data?.book?.id ?? '';
    if (!createdId) {
      setError(json?.message || 'Failed to create book.');
      return;
    }

    currentBookId.value = createdId;
    book.value = {
      ...makeEmptyBook(),
      id: createdId,
      title: payload.title ?? '',
...
const metadataSearchQuery = computed(() => {
    ...
    <div class="w-full h-full p-4 overflow-auto">
      ...
      >
        ...
        <div v-if="book" class="flex items-center justify-between gap-4">
          <div class="min-w-0">
            <div v-if="book.id" class="text-sm opacity-70">
              ID: <span class="font-mono">{{ book.id }}</span>
            </div>
            <div v-else class="text-sm opacity-70">New book</div>
          </div>
          ...
          <div class="grid gap-4">
            ...
            <div class="grid gap-2">
              <div class="flex items-center gap-1">
                ...
                <span
                  v-if="titleMissing"
                  class="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded border border-(--error-color) text-(--error-color)"
                >
                  Required
                </span>
                <icon
