...
const autoLimit = computed(() => {
    ...
    const rows = Math.max(
        1,
        Math.ceil((containerH + gapPx) / (cardHeightPx + gapPx)) +
            bufferRows.value,
    );
        ...
        <div class="p-4 border-b border-base-300">
            <div class="flex items-center justify-between gap-4">
                <div class="flex flex-col">
                    ...
                    <p class="text-sm opacity-70">
                        Incremental harness for /api/books cursor pagination +
                        infinite scroll.
                    </p>
                </div>
            ...
            <div v-if="showDebug" class="mt-4 text-sm">
                <div class="flex flex-wrap gap-x-6 gap-y-1">
                    ...
                    <div>
                        <span class="opacity-70">loading:</span> {{ loading }}
                    </div>
                    <div>
                        <span class="opacity-70">hasMore:</span> {{ hasMore }}
                    </div>
                    <div>
