...
import { bookFiles, books, collectionBooks, collectionMembers } from "~/utils/db/schema";
...
export default defineEventHandler(async (event) => {
  ...
  const memberCollectionIds = Array.from(new Set(memberships.map((m) => m.collectionId))).filter(
    Boolean,
  );
  ...
  const visible = await cloudDb
    .select({ id: books.id })
    .from(books)
    .innerJoin(collectionBooks, and(eq(collectionBooks.bookId, books.id), eq(books.id, id)))
  ...
  const files = await cloudDb.select().from(bookFiles).where(eq(bookFiles.bookId, id));
