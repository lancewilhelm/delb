...
import { series } from '~/utils/db/schema';
 ...
 *
 * Admin-only endpoint for series typeahead.
 *
 * Lean behavior:
...
export default defineEventHandler(async (event) => {
  logger.debug('GET /api/series/search');
  ...
  try {
    ...
    logger.error(error, 'GET /api/series/search: failed');
    throw createError({
      statusCode: 500,
      statusMessage: 'Failed to search series',
    });
  }
});
