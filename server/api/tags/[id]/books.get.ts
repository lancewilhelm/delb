...
/**
 * GET /api/tags/:id/books?collectionId=<optional>&limit=<n>&cursor=<opaque>
 *
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!memberCollectionIds.length) {
      return { success: true, data: { tag, books: [], nextCursor: null } };
    }
    ...
    // 3) Page query: visible books with this tag in scoped collections.
    // Sort: newest first, deterministic by id. Cursor is (createdAt,id).
    const pageRows = await cloudDb
      .select({ book: books })
      .from(books)
      .innerJoin(bookTags, eq(bookTags.bookId, books.id))
    ...
    // 5) Attach publisher + series display info
    ...
    return { success: true, data: { tag, books: out, nextCursor } };
  } catch (error: unknown) {
